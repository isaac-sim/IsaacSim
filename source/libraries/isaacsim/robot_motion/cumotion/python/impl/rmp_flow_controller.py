...
class RmpFlowController(mg.BaseController):
    ...
    def forward(
        ...
        if self._rmp_flow is None:
            get_logger().warning(
                "RmpFlowController: forward failed because reset() has not been called or reset failed.",
                capture_source=True,
            )
            return None
    ...
    def reset(
        ...
        if current_joint_positions is None:
            get_logger().warning(
                "RmpFlowController: reset failed because required controlled joint positions "
        ...
        if self._rmp_flow is None:
            get_logger().warning(
                "RmpFlowController: reset failed because cuMotion RMPflow could not be initialized.",
                capture_source=True,
            )
