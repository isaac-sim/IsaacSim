...
class TrajectoryFollower(BaseController):
    ...
    def reset(self, estimated_state: RobotState, setpoint_state: RobotState | None, t: float, **kwargs: object) -> bool:
        ...
        if self._trajectory is None:
            log.warning("TrajectoryFollower: reset failed because no trajectory has been set.", capture_source=True)
            return False
    ...
    def forward(
        ...
        if (self._trajectory is None) or (self._start_time is None):
            log.warning(
                "TrajectoryFollower: forward failed because no trajectory is set or reset() was not called.",
                capture_source=True,
            )
            return None
        ...
        if trajectory_time < 0.0:
            # The clock is stale (time cannot move backwards).
            self._start_time = None
            self._trajectory = None
            log.warning(
                f"TrajectoryFollower: forward failed because trajectory time {trajectory_time} "
                "is before the start time (stale clock).",
                capture_source=True,
            )
            return None
