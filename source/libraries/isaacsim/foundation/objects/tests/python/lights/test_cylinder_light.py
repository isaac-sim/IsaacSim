...
import pytest
import warp as wp
from isaacsim.foundation.objects import CylinderLight as Light
...
def _get_paths(stage: Any, populate: Any) -> Any:
    ...
    if populate:
        for path in paths:
            stage.define_prim(path, "CylinderLight")
        paths = "/World/Prim.*"
    return paths
...
def test_radii(capsys: Any, stage: Any, populate: Any, values: Any) -> None:
    """Test radii.
...
@pytest.mark.parametrize("populate", [True, False])
def test_enabled_treat_as_lines(capsys: Any, stage: Any, populate: Any, values: Any) -> None:
    """Test enabled treat as lines.
    ...
    prims = Light(_get_paths(stage, populate))
    # get values
    output = prims.get_enabled_treat_as_lines()
    isaacsim_test.check_array(output, shape=(5, 1), dtype=wp.bool)
    # round-trip values
    prims.set_enabled_treat_as_lines(values)
    output = prims.get_enabled_treat_as_lines()
    isaacsim_test.check_array(output, shape=(5, 1), dtype=wp.bool)
...
def test_are_of_type(capsys: Any, stage: Any) -> None:
    ...
    Args:
        ...
        stage: Stage used by the test.
    """
    type_names = ["CylinderLight", "SphereLight", "Scope"]
