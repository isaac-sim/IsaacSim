...
@pytest.mark.parametrize("populate", [True, False])
def test_enabled_treat_as_points(capsys: Any, stage: Any, populate: Any, values: Any) -> None:
    ...
    prims = Light(_get_paths(stage, populate))
    # get values
    output = prims.get_enabled_treat_as_points()
    isaacsim_test.check_array(output, shape=(5, 1), dtype=wp.bool)
    # round-trip values
    prims.set_enabled_treat_as_points(values)
    output = prims.get_enabled_treat_as_points()
    isaacsim_test.check_array(output, shape=(5, 1), dtype=wp.bool)
...
def test_are_of_type(capsys: Any, stage: Any) -> None:
    ...
    Args:
        ...
        stage: Stage used by the test.
    """
    type_names = ["SphereLight", "DiskLight", "Scope"]
