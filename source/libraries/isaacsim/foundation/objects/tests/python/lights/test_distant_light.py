...
import pytest
import warp as wp
from isaacsim.foundation.objects import DistantLight as Light
...
def _get_paths(stage: Any, populate: Any) -> Any:
    ...
    if populate:
        for path in paths:
            stage.define_prim(path, "DistantLight")
        paths = "/World/Prim.*"
    return paths
...
def test_angles(capsys: Any, stage: Any, populate: Any, values: Any) -> None:
    """Test angles.
...
def test_are_of_type(capsys: Any, stage: Any) -> None:
    ...
    Args:
        ...
        stage: Stage used by the test.
    """
    type_names = ["DistantLight", "SphereLight", "Scope"]
