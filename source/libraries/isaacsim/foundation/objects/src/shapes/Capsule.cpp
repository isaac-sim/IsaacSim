...
Capsule::Capsule(const std::variant<std::string, std::vector<std::string>>& paths,
...
void Capsule::setRadii(const array::Array& radii, const std::optional<array::Array>& indices)
{
...
array::Array Capsule::getRadii(const std::optional<array::Array>& indices)
{
    return std::get<array::Array>(this->getAttributeValues("radius", indices));
}

void Capsule::setHeights(const array::Array& heights, const std::optional<array::Array>& indices)
{
    int64_t batchSize = _resolveIndexedSize(indices);
    this->setAttributeValues("height", heights.broadcastTo(array::Shape({ batchSize, int64_t{ 1 } })), indices);
}

array::Array Capsule::getHeights(const std::optional<array::Array>& indices)
{
    return std::get<array::Array>(this->getAttributeValues("height", indices));
}

void Capsule::setAxes(const std::variant<std::string, std::vector<std::string>>& axes,
                      const std::optional<array::Array>& indices)
{
    this->setAttributeValues("axis", _resolveStringList(_resolveAxes(axes), indices), indices);
}

std::vector<std::string> Capsule::getAxes(const std::optional<array::Array>& indices)
{
