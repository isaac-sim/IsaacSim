...
Cylinder::Cylinder(const std::variant<std::string, std::vector<std::string>>& paths,
...
void Cylinder::setRadii(const array::Array& radii, const std::optional<array::Array>& indices)
{
...
array::Array Cylinder::getRadii(const std::optional<array::Array>& indices)
{
    return std::get<array::Array>(this->getAttributeValues("radius", indices));
}

void Cylinder::setHeights(const array::Array& heights, const std::optional<array::Array>& indices)
{
    int64_t batchSize = _resolveIndexedSize(indices);
    this->setAttributeValues("height", heights.broadcastTo(array::Shape({ batchSize, int64_t{ 1 } })), indices);
}

array::Array Cylinder::getHeights(const std::optional<array::Array>& indices)
{
    return std::get<array::Array>(this->getAttributeValues("height", indices));
}

void Cylinder::setAxes(const std::variant<std::string, std::vector<std::string>>& axes,
                       const std::optional<array::Array>& indices)
{
    this->setAttributeValues("axis", _resolveStringList(_resolveAxes(axes), indices), indices);
}

std::vector<std::string> Cylinder::getAxes(const std::optional<array::Array>& indices)
{
