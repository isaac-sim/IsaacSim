                                                  ...
                                                  bool logWarningOnFallback = false)
{
    ...
    if (useFabricHierarchy && usdrtStage)
    {
        ...
        if (settings->getAsBool("/app/useFabricSceneDelegate"))
        {
            ...
            // - Cached transform (with explicit update call)
            {
                ...
                if (!usdrtStage->HasPrimAtPath(pathString))
                {
                    if (logWarningOnFallback)
                    {
                        CARB_LOG_WARN(
                            "Prim at path: '%s' is not available in USDRT. Falling back to non-IFabricHierarchy call",
                            pathString.c_str());
                    }
                    return computeWorldXformNoCache(usdStage, usdrtStage, path, timecode, false);
                }
                auto worldMatrixAttr = usdrtStage->GetPrimAtPath(pathString).GetAttribute("omni:fabric:worldMatrix");
    ...
    if (usdrtStage && usdrtStage->HasPrimAtPath(path.GetString()))
    {
